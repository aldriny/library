...
<div class="d-flex justify-content-center align-items-center mt-3" style="min-height: 70vh;">
    <div class="col-md-4">
        <div class="card-body">
            <h2 class="card-title text-center mb-4">Add New Author</h2>
            <form action="{{ route('createAuthor') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label for="name">Author Name</label>
                    <input type="text" class="form-control" name="name" id="name" placeholder="Enter author name" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="biography" id="bio" placeholder="Enter biography">{{ old('biography') }}</textarea>
                    @error('biography')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
