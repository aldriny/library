            ...
            <div class="card-body">
                ...
                <form action="{{ route('editBook', ['id' => $book->id]) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" id="description" placeholder="Enter book description">{{ old('description', $book->description) }}</textarea>
                        @error('description')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="price" id="price"
                            placeholder="Enter book price" value="{{ old('price', $book->price) }}">
                        @error('price')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="language" name="language">
                            ...
                            <option value="english" {{ old('language', $book->language) == 'english' ? 'selected' : '' }}>
                                English</option>
                            <option value="arabic" {{ old('language', $book->language) == 'arabic' ? 'selected' : '' }}>
                                Arabic</option>
                            <option value="spanish" {{ old('language', $book->language) == 'spanish' ? 'selected' : '' }}>
                                Spanish</option>
                        </select>
                        @error('language')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div id="addCategoryForm" class="mt-3" style="display: none; opacity: 0; transition: opacity 0.5s;">
                        <div class="p-3"
                            style="background-color: #f8f9fa; border: 1px solid #ced4da; border-radius: 5px;">
                            <div class="form-group">
